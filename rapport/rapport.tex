...
\\Les méthodes classiques étudiés auparravant ou présenté dans la littérature, nottemment l'amélioration de la qualité d'images (filtres, etc) ou la reconnaissance des formes complexes, peuvent ne pas répondre à ce type de problématique.
\\Ainsi dans ce projet, nous allons explorer un autre axe de recherche basé sur les méthodes de réseaux de neuronnes qui on prouvait leur efficacité à reconnaître et classifier les formes les plus complexes.
\\Cette recherche vas nous permettre de mettre en oeuvre nos compétences en traitement d'image et d'explorer de nouvelle méthodes plus avancé.
		...
		\subsection{Objectifs}
			...
			Dans notre étude nous allons considéré 26 lettres de l'alphabet latin en majuscules et présenter des résultats préliminaires
...
%Diagramme de Cas d'Utilisation à la page \pageref{fig:useCaseDiagram}. 
			...
			\begin{figure}[H]
				...
				\label{fig:recon}
			\end{figure}
			\paragraph{} Après établir le prétraitement détaillé ci-dessus, on va procéder a la segmentation des caractères.
			% \paragraph{} La segmentation des caractères 
			% \paragraph{}Notre programme va segmenter les charactères, en faire des imagettes et donner ces imagettes individuellement à YOLO. Ensuite après cette reconnaissance, l'IHM présente le résultat.
			\paragraph{} Dans un premier temps, une projection d'histogramme vertical de l'image binaire est calculée ; cette projection d'histogramme vertical correspond à un vecteur de taille égale à la largeur de l'image et sommant les pixels blancs et noirs par colonne.% Par la suite, nous avons écrit une fonction pour créer une image pour l'histogramme précédent.
Les images d'entrée sont généralement bruitées, ce qui provoque l'apparition de quelques petits pics indésirables dans l'image de l'histogramme de projection. Afin de supprimer cet effet de bruit et de supprimer ces pics, nous avons créé une fonction de seuillage.
Ensuite, et en se basant sur l'analyse de l'image de projection de l'histogramme vertical, nous avons fait
écrire une fonction pour compter le nombre de caractères et les localiser. Le critère de détection est basé sur la succession de projections nulles et non nulles. En sortie, nous avons le nombre de caractères et les coordonnées (début, fin) de chacun. Par conséquent, nous implémentons la fonction qui réalise la segmentation du mot entier.
