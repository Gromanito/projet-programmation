...
def coordonneeLigne(myprojection):
    ...
    for i, value in enumerate(myprojection):
        if value!=0:
            if not danslaligne or (danslaligne and i == len(myprojection)): coordDF.append(i)
            danslaligne = True
        else : 
...
def coordonneeCaractere(myprojection):
    ...
    j = 0
    print(espaceEntreLettre)
    print(moyEspace, " ", moyEspace*0.2)
    while (espaceEntreLettre != []):
        if espaceEntreLettre[0] < moyEspace*0.2:
            D = lettres.pop(j)
            F = lettres.pop(j)
...
# # ----------- TEST --------------
# image = cv2.imread("aSegmenterAvecUneLettrePasBienAttache.png")
# _,_,_,img = segmentation(image)
# for i in range(len(img[0])):
#     cv2.imshow("image",img[0][i])
#     cv2.waitKey(0)